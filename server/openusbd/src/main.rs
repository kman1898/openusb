...
async fn main() -> Result<()> {
    ...
    if config.security.mode != "open" && config.security.jwt_secret == "openusb-change-this-secret"
    {
        tracing::warn!(
            "Using default JWT secret! Set 'jwt_secret' in [security] config for production."
        );
    }
